...
setwd("~/Masters/Project/sfs_tests")

############## Code for hetrozygosity plot with CIs ##############
...
group <- c("Dog - Breed", "Dog - Breed", "Wolf", "Dog - Non-breed", "Dog - Non-breed", "Wolf", "Wolf", "Wolf", "Wolf", "Wolf")

## Create dataframe of samples, heterozygosity values and CIs:
het_df <- data.frame(all_samples, I(group), all_het, all_lower, all_upper)
colnames(het_df) <- c("Sample", "Group", "Heterozygosity", "LowerCI", "UpperCI")

## Order dataframe by group:
het_df <- het_df[order(het_df$Group),]
het_df$Sample <- factor(het_df$Sample, levels = het_df$Sample)
het_df

## Plot graph with ggplot: 
# Set colours:
cols = wes_palette("IsleofDogs1", n = 10, type = "continuous") # Create palette of specified size (will take first 3 colours in palette of 10 colours)

# Plot graph:
ggplot(data = het_df, aes(x = Sample, y = Heterozygosity, fill = Group)) + geom_bar(stat = "Identity") +
  scale_fill_manual(values = cols) + # Specify colours
  geom_errorbar(aes(ymin = LowerCI, ymax = UpperCI), width = 0.2) + # Add error bars
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + # Sample names vertical
  ggtitle("Heterozygosity estimates across 10 canine samples") + # Add title 
